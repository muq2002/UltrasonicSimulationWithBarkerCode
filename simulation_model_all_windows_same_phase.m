clear all; clc;
%% Ultrasounic Model Without Barker Code (Pulse Comparsion)
%% Ultrasounic Model: 5khz
f1 = 2.5e3;
t1 = linspace(0, 10/f1, 2*f1);

tx_signal = sin(2*pi*f1*t1);
...
% Plot result
figure(2);
subplot(2,1, 1); plot(t2, us_window_5, '-r'); title('RX Window at 5KHz'); xlabel('Time'); ylabel('Amp.'); ylim([-2 2]);
subplot(2,1, 2); plot(corr_window(20200:end), '-r');
